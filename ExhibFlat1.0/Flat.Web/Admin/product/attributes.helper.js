...
$(document).ready(function () {
    shortskustr = document.getElementById('ctl00_contentHolder_txtProductShortName').value;
    ...
    $("#ctl00_contentHolder_txtProductName").bind("keyup", function () {
        var pname = $("#ctl00_contentHolder_txtProductName").val();
        CheckProductNameAndCode("#ctl00_contentHolder_txtProductNameRepeat", "产品名称已存在,请修改.", pname, "");
    });
    $("#ctl00_contentHolder_txtProductCode").bind("keyup", function () {
        var pcode = $("#ctl00_contentHolder_txtProductCode").val();
        CheckProductNameAndCode("#ctl00_contentHolder_txtProductCodeRepeat", "产品编号已存在,请修改.", "", pcode);
    });
    $("#ctl00_contentHolder_txtProductShortName").bind("keyup", function () {
        $(".SpecificationTr .skuItem_ShortSku").each(function () {
            var rowid = $(this).attr("id").split('_')[1];
            var ssku = getRowHtml(rowid);
            $(this).val(ssku);
        });
    });
    init();
...
function CheckProductNameAndCode(controlname, tip, pname, pcode) {
    var pid = GetQueryString("productId");
    $.ajax({
        url: '/API/CommonHandler.ashx?op=CheckProductName',
        dataType: 'json',
        type: 'GET',
        timeout: 5000,
        data: { productname: pname, productcode: pcode, productid: pid },
        error: function (xm, msg) {
            alert(msg);
        },
        success: function (data) {
            if (data.res == "true") {
                $(controlname).text(tip);
            } else
                $(controlname).text("");
        }
    });
...
function prepareAttributes(attributes) {
    ...
    $.each(attributes, function (i, attribute) {
        ...
        var lirLDiv = $("<div style=\"float:left\"></div>");
        var titleSpan = $(String.format("<span class=\"sL\" style=\"min-width:80px;\">{0}：<\/span>", attribute.Name));
        liLDiv.append(titleSpan);

        if (attribute.UsageMode == "1") {

            var selectTag = $(String.format("<select id=\"attribute{0}\" class=\"formselects\"><\/select>", attribute.AttributeId));
            ...
            //如果这个商品属性可以自定义,显示自定义编辑框
            if (attribute.IS_INPUT_PROP == true && attribute.IS_ALLOW_ALIAS == false) {
                lirLDiv.append(liLDiv);
                lirLDiv.append($("<div id=\"div_" + attribute.AttributeId + "\" style=\"display:none;float:left;margin-left:10px;\"><input type=\"text\" id=\"txtvalue" + attribute.AttributeId + "\"/><input type=\"button\" value=\"保存\" onclick=\"javascript:AddValue('" + attribute.AttributeId + "','" + attribute.UsageMode + "',this)\"/></div>"));
                liTag.append(lirLDiv);
                liTag.append($("<div id=\"divEx_" + attribute.AttributeId + "\" style=\"float:left;\"><input type=\"button\" value=\"+\" onclick=\"javascript:ShowAttriBox('" + attribute.AttributeId + "')\"/><input type=\"text\" id=\"txtvalueEx" + attribute.AttributeId + "\" value=\"\" style=\"display:none;\" /></div>"));
                ulTag.append(liTag);
            }
            else {
                liTag.append(liLDiv);
...
//显示自定义商品属性框
function ShowAttriBox(attributeId) {
    if ($("#txtvalueEx" + attributeId).css("display") == "none") {
        $("#txtvalueEx" + attributeId).show();
        $("#attribute" + attributeId).prop("disabled", true);

    }
    else {
        $("#txtvalueEx" + attributeId).hide();
...
function restoreState() {
    if ($("#ctl00_contentHolder_txtAttributes").val().length > 0) {
        ...
        $.each(selectedAttributes, function (itemIndex, itemNode) {
            ...
            if ($(itemNode).attr("usageMode") == "0") {
                ...
                var attributeInput = $("#txtvalueEx" + attributeId);
                if ($(attributeControl).length > 0) $(attributeControl).val($(itemNode).children().eq(0).attr("valueId"));
                if ($(attributeInput).length > 0 && $(itemNode).children().eq(0).attr("CUS_Name") != "") {
                    $(attributeInput).val($(itemNode).children().eq(0).attr("CUS_Name")).show();
                    $(attributeControl).prop("disabled", true);
                }
            }
        });
    }
    ...
    if ($("#ctl00_contentHolder_chkSkuEnabled").attr("checked") == "checked") {
        ...
        $.each(selectedSkus, function () {
            ...
            $("#barcode_" + rowIndex).val($(this).attr("barcode"));
            $("#GBCode_" + rowIndex).val($(this).attr("gbcode"));
            $("#skusubname_" + rowIndex).val($(this).attr("skusubname"));
            $("#skuimage_" + rowIndex).attr("src", $(this).attr("skuimage"));
            $("#skuimage_" + rowIndex).css("display", "");
            //$("#skuimage_" + rowIndex).val($(this).attr("skuimage"));
            $.each($(this).find("sku"), function () {
                var attributeId = $(this).attr("attributeId");
                var valueId = $(this).attr("valueId");
                selectSkuValue(rowIndex, attributeId, valueId, $("span[class='sku" + attributeId + "values'][valueId='" + valueId + "']").find("span").text());
                //$("#shortSku_" + rowIndex).html(shortskustr);
            });
...
function prepareSkus() {
    ...
    for (cellIndex = 0; cellIndex < cellFields.length; cellIndex++) {
        ...
        fillSkuBox(skuBox, skuId, skuItem.SKUValues, skuItem.IS_ALLOW_ALIAS, skuItem.IS_INPUT_PROP);
        skuTableHolder.append(skuBox);
    }

    tableHeader.append($("<td align=\"center\">SKU编码<\/td>"));
    tableHeader.append($("<td align=\"center\">国标码<\/td>"));
    tableHeader.append($("<td align=\"center\">商品条形码<\/td>"));
    tableHeader.append($("<td align=\"center\"><em >*<\/em>商品SKU简称<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:130px\"><em >*<\/em>批发价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:100px\"><em >*<\/em>成本价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:150px;display:none;\" ><em >*<\/em>分销商采购价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:100px\"><em >*<\/em>库存<\/td>"));
    tableHeader.append($("<td align=\"right\" style=\"width:100px\"><em >*<\/em>警戒库存<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:100px\"><em >*<\/em>重量(克)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:100px\">SKU图片<\/td>"));
...
function fillSkuBox(box, skuId, skuValues, IS_ALLOW_ALIAS, IS_INPUT_PROP) {
    var valueList = $(String.format(".sku{0}values", skuId));
    if (IS_ALLOW_ALIAS == "1" && IS_INPUT_PROP == "1")//可自定义，备注
    {
        $.each(skuValues, function (valIndex, val) {
            if (val.ValueType == "Custom") {
                box.append($(String.format("<span valueId=\"{0}\" class=\"sku{1}values\" style=\"padding:3px;\" id={2}><span>{3}</span><a href=\"javascript:;\" onclick=\"EditAttr({4},2);\">✎</a> <a href=\"javascript:;\" onclick=\"deleteCustomAttr({5});\">×</a><\/span>", val.ValueId, skuId, val.ValueId, val.ValueStr, val.ValueId, val.ValueId)).attr("AttrType", "3"));
            }
            else if (val.ValueType == "Memo") {
                box.append($(String.format("<span valueId=\"{0}\" class=\"sku{1}values\" style=\"padding:3px;\" id={2}><span>{3}</span><a href=\"javascript:;\" onclick=\"EditAttr({4},0);\">✎</a><\/span>", val.ValueId, skuId, val.ValueId, val.ValueStr, val.ValueId)).attr("AttrType", "2"));
            }
                //else if (val.ValueType == "Alias") {
                //    box.append($(String.format("<span valueId=\"{0}\" class=\"sku{1}values\" style=\"padding:3px;\" id={2}>{3}<a href=\"javascript:;\" onclick=\"EditAttr({4},1);\">✎</a><\/span>", val.ValueId, skuId, val.ValueId, val.ValueStr, val.ValueId)));
                //    box.attr("AttrType", "2");
                //}
            else {
                box.append($(String.format("<span valueId=\"{0}\" class=\"sku{1}values\" style=\"padding:3px;\" id={2}><span>{3}</span><a href=\"javascript:;\" onclick=\"EditAttr({4},0);\">✎</a><\/span>", val.ValueId, skuId, val.ValueId, val.ValueStr, val.ValueId)));
            }
        });
        box.append($(String.format("<b class=\"specspan\" style=\"padding:3px;\"><input id=addAttr_{0} style=\"width:60px;\" type=\"text\" /><a href=\"javascript:;\" onclick=\"addCustomAttr({1});\">＋</a></b>", skuId, skuId)));
    }
    else if (IS_ALLOW_ALIAS == "1" && IS_INPUT_PROP == "0")//可别名
    {
        $.each(skuValues, function (valIndex, val) {
            if (val.ValueType == "Alias") {
                box.append($(String.format("<span valueId=\"{0}\" class=\"sku{1}values\" style=\"padding:3px;\" id={2}><span>{3}</span><a href=\"javascript:;\" onclick=\"EditAttr({4},1);\">✎</a><\/span>", val.ValueId, skuId, val.ValueId, val.ValueStr, val.ValueId)).attr("AttrType", "1"));
            }
            else {
                box.append($(String.format("<span valueId=\"{0}\" class=\"sku{1}values\" style=\"padding:3px;\" id={2}><span>{3}</span><a href=\"javascript:;\" onclick=\"EditAttr({4},1);\">✎</a><\/span>", val.ValueId, skuId, val.ValueId, val.ValueStr, val.ValueId)));
            }
    ...
    {
        $.each(skuValues, function (valIndex, val) {
            box.append($(String.format("<span valueId=\"{0}\" class=\"sku{1}values\" style=\"padding:3px;\"><span>{2}</span><\/span>", val.ValueId, skuId, val.ValueStr)));
        });
...
//属性操作
//添加自定义属性
function addCustomAttr(skuId) {
    var temp = false;
    var box = $("#skuBox_" + skuId);
    var boxlist = $("#skuBox_" + skuId + " span span");
    var inbox = $("#addAttr_" + skuId);
    var newv = inbox.val().replace(/(^\s*)|(\s*$)/g, '');
    $.each(boxlist, function (valindex, val) {
        if ($(this).html().toUpperCase() == newv.toUpperCase()) {
            alert("存在同名属性！");
            inbox.val(""); temp = true; return;
        }
    });
    if (!temp) {
        if (newv.length > 0 && newv.length < 15) {
            var id = parseInt(Math.random() * 99999999 + 100000);//生成一个从 100000 - 999999 之间的随机整数
            var obj = $(String.format("<span valueId=\"{0}\" class=\"sku{1}values specspan\" style=\"padding:3px;\" id={2} AttrType='3'><span>{3}</span><a href=\"javascript:;\" onclick=\"EditAttr({4},2);\">✎</a> <a href=\"javascript:;\" onclick=\"deleteCustomAttr({5});\">×</a><\/span>", id, skuId, id, newv, id, id));
            $(obj).find("span").unbind("click").bind("click", function () { selectSkuValue(CustomnewRowIndex, skuId, id, newv); setSkuCode(CustomnewRowIndex); });
            //$(obj).click(function () { selectSkuValue(rowId, skuId, id, v); });
            box.prepend(obj);
            inbox.val("");
        } else {
            alert("添加内容不能为空或大于15个字符！");
        }
    }
}
//删除自定义属性
function deleteCustomAttr(id) {
    var box = $("#" + id);
    box.remove();
    updateAllAttr(id, "请选择", 1, 0);
}

//更新自定义属性
function updateCustomAttr(boxid, v) {
    var box = $("#" + boxid);
    var inbox = $("#i_" + boxid);
    var newv = inbox.val().replace(/(^\s*)|(\s*$)/g, '');
    if (newv.length > 0 && v.length < 15) {
        box.html("<span>" + newv + "</span><a href=\"javascript:;\" onclick=\"EditAttr(" + boxid + ",2);\">✎</a> <a href=\"javascript:;\" onclick=\"deleteCustomAttr(" + boxid + ");\">×</a>");
        updateAllAttr(boxid, newv, 0, 3);
    } else {
        alert("更新内容不能为空或大于15个字符！");
    }
    var skuid = box.parent().attr("id");
    $(box).find("span").unbind("click").bind("click", function () { selectSkuValue(CustomnewRowIndex, skuid.substr(7, skuid.length), $(this).parent().attr("valueId"), $(this).html()); setSkuCode(CustomnewRowIndex); });
}
//更新属性备注
function updateAttrMemo(boxid, v) {
    var box = $("#" + boxid);
    var inbox = $("#i_" + boxid);
    var newv = inbox.val().replace(/(^\s*)|(\s*$)/g, '');
    if (newv.length <= 0) {//备注无值
        //原生自带
        box.html("<span>" + v + "</span><a href=\"javascript:;\" onclick=\"EditAttr(" + boxid + ",0)\">✎</a>");
        updateAllAttr(boxid, v, 0, 0);
        //$(inbox).find("span").bind("click", function () {selectSkuValue });
    }
    else {
        if (newv.length < 15) {
            box.html("<span>" + v + "(" + newv + ")</span><a href=\"javascript:;\" onclick=\"EditAttr(" + boxid + ",0);\">✎</a>");
            updateAllAttr(boxid, v + "(" + newv + ")", 0, 2);
        } else {
            alert("更新内容不能大于15个字符！");
        }
    }
...
//更新属性别名
function updateAttrAlias(boxid, v) {
    var box = $("#" + boxid);
    var inbox = $("#i_" + boxid);
    var newv = inbox.val().replace(/(^\s*)|(\s*$)/g, '');
    if (newv.length > 0 && newv.length < 15) {
        box.html("<span>" + newv + "</span><a href=\"javascript:;\" onclick=\"EditAttr(" + boxid + ",1);\">✎</a>");
        updateAllAttr(boxid, newv, 0, 1);
    } else {
        alert("更新内容不能为空或大于15个字符！");
    }
    var skuid = box.parent().attr("id");
...
//打开编辑属性
function EditAttr(boxid, status) {
    var box = $("#" + boxid);
    var boxval = box.find("span").text();
    var finindex = boxval.indexOf("(");
    var linindex = boxval.indexOf(")");
    var v = boxval.substr(finindex + 1, linindex - finindex - 1);
    if (finindex == -1) finindex = 0;
    var v1 = boxval.substring(0, boxval.length - linindex + finindex - 1);
    if (status == 0)//备注
    {
        box.html($(String.format("<input id=\"i_{0}\" type=\"text\" style=\"width:60px;\" value=\"{1}\" /><a href=\"javascript:;\" onclick=\"updateAttrMemo({2},'{3}');\">√</a>", boxid, v, boxid, v1)));
    }
    else if (status == 1)//别名
    {
        box.html($(String.format("<input id=\"i_{0}\" type=\"text\" style=\"width:60px;\" value=\"{1}\"/><a href=\"javascript:;\" onclick=\"updateAttrAlias({2},'{3}');\">√</a>", boxid, v1, boxid, v1)));
    }
    else {//自定义
        box.html($(String.format("<input id=\"i_{0}\" type=\"text\" style=\"width:60px;\" value=\"{1}\"/><a href=\"javascript:;\" onclick=\"updateCustomAttr({2},'{3}');\">√</a>", boxid, v1, boxid, v1)));
    }
}

function updateAllAttr(valueid, newvalue, mode, attrtype) {
    var skuidlist = $("div[skuid]");
    if (mode == 1)
        $.each(skuidlist, function (valindex, val) {
            if ($(this).attr("valueid") == valueid) {
                $(val).html(newvalue);
                $(val).attr("class", "specdefault");
                $(val).attr("valueid", "");
                $(val).attr("AttrType", attrtype);
                var rowid = $(val).attr("rowid");
                var ssku = getRowHtml(rowid);
                var displayshortsku = $(String.format("#ShortSku_{0}", rowid));
                $(displayshortsku).val(ssku);
            }
        });
    else
        $.each(skuidlist, function (valindex, val) {
...
function addItem() {
    ...
    dataRow.append(createGBCode(newRowIndex));
    dataRow.append(createBarCode(newRowIndex));
    dataRow.append(createShortSku(newRowIndex));
    ...
    dataRow.append(createUploadCell(newRowIndex)); //上传图片按钮
...
function UploadSKUimg(skuId, rowindex) {
    var pathurl = "product/UploadSKUImg.aspx?skuId=" + skuId + "&rowindex=" + rowindex;
    var title = "SKU图片";
    DialogFrame(pathurl, title, 300, 200);
}
...
function adjustSkuBox() {
    ...
    CustomnewRowIndex = rowId;
    var skuId = $(this).attr("skuId");
    var skuBox = $("#skuBox_" + skuId);
    var valueList = $(String.format(".sku{0}values", skuId));

    $.each(valueList, function (i, listItem) {
        $(listItem).find("span").unbind("click"); //因为每个规格都是用的同一个弹出层，所以必须先取消上一次事件绑定
        if (checkValue(rowId, skuId, $(this).attr("valueId"))) {
            $(listItem).addClass("specsna");
        } else {
            $(listItem).addClass("specspan").removeClass("specsna");
            $(listItem).find("span").bind("click", function () { selectSkuValue(rowId, skuId, $(this).parent().attr("valueId"), $(this).html()); setSkuCode(rowId); });
        }
    });
}
...
function selectSkuValue(rowId, skuId, valueId, valueStr) {
    ...
    shortskustr += "_" + valueStr;
    $(displayCtl).attr("valueId", valueId);
    $(displayCtl).addClass("specdiv").removeClass("specdefault");
    var AttrTypeValue = $("#" + valueId).attr("AttrType");
    if (typeof (AttrTypeValue) == "undefined")
        AttrTypeValue = "0";
    $(displayCtl).attr("AttrType", AttrTypeValue);
    var rowIdentity = getRowIdentity(rowId);
    //给商品sku简称赋值：
    var ssku = getRowHtml(rowId);
    var displayshortsku = $(String.format("#ShortSku_{0}", rowId));
    $(displayshortsku).val(ssku);
    //
    //var displayskuCode = $(String.format("#skuCode_{0}", rowId));

    //if ($(displayskuCode).val() == '' || $(displayskuCode).val().indexOf('-')<0)
    //{
    //    var sskucode = getRowSKUCode(rowId);
    //    $(displayskuCode).val(sskucode);
    //}
...
function getRowSKUCode(rowId) {
    var rowSKUCode = "";
    var rowIdentity = "";
    for (index = 0; index < cellFields.length; index++) {
        rowIdentity += $(String.format("#skuDisplay_{0}_{1}", rowId, cellFields[index])).attr("valueId") + "-";
    }
    var rowprocode = $("#ctl00_contentHolder_txtGeneralCode").val();
    $.ajax({
        url: '/API/CommonHandler.ashx?op=CreateSKUCode',
        dataType: 'json',
        type: 'GET',
        async: false,
        data: { attjoincode: rowIdentity, procode: rowprocode },
        error: function (xm, msg) {
            alert(msg);
        },
        success: function (data) {
...
function getRowHtml(rowId) {
    var rowIdentity = $("#ctl00_contentHolder_txtProductShortName").val();
    for (index = 0; index < cellFields.length; index++) {
        var value = $(String.format("#skuDisplay_{0}_{1}", rowId, cellFields[index])).html();
        var fin = value.indexOf("(");
        var lin = value.indexOf(")");
        if (fin != -1)
            rowIdentity += "_" + value.substr(fin + 1, lin - fin - 1);
        else
            rowIdentity += "_" + value;
    }
    return rowIdentity;
...
function createSalePriceCell(rowIndex) {
    ...
    var btnSkuMemberPrice = $("<input type=\"button\" value=\"分销商价\" onclick=\"editSkuMemberPrice(" + rowIndex + ");\" \/>");
...
function createBarCode(rowIndex) {
    var cell = createCell();
    var barcodeCell = $(String.format("<input type=\"text\" class=\"skuItem_BarCode\" id=\"barcode_{0}\"  style=\"width:120px;\" \/>", rowIndex));
    $(barcodeCell).val($("#ctl00_contentHolder_txtProBarCode").val());
    $(barcodeCell).bind("blur", function () { onBarCodeblur($(barcodeCell).val()) });
    cell.append(barcodeCell);
    return cell;
}

function createGBCode(rowIndex) {
    var cell = createCell();
    var GBCodeCell = $(String.format("<input type=\"text\" class=\"skuItem_GBCode\" id=\"GBCode_{0}\"   style=\"width:120px;\" \/>", rowIndex));

    $(GBCodeCell).bind("blur", function () { onGBCodeblur($(GBCodeCell).val()); });
    cell.append(GBCodeCell);
    return cell;
}

function onBarCodeblur(barcode) {
    var pid = GetQueryString("productId");
    $.ajax({
        url: '/API/CommonHandler.ashx?op=CheckBarCode',
        dataType: 'json',
        type: 'GET',
        async: false,
        data: { barcode: barcode, productid: pid },
        error: function (xm, msg) {
            alert(msg);
        },
        success: function (data) {
...
function createShortSku(rowIndex) {
    var cell = createCell();
    var ShortSkuCell = $(String.format("<input type=\"text\" class=\"skuItem_ShortSku\" id=\"ShortSku_{0}\" style=\"width:120px;\" \/>", rowIndex));
    $(ShortSkuCell).val($("#ctl00_contentHolder_txtShortSku").val());
    $(ShortSkuCell).bind("blur", function () { onShortSkublur(rowIndex); });
    cell.append(ShortSkuCell);
    return cell;
}

function onShortSkublur(rowId) {
    var displayshortsku = $(String.format("#ShortSku_{0}", rowId));
    var curshorsku = displayshortsku.val();
    $(displayshortsku).val(curshorsku.length > 50 ? curshorsku.substr(0, 50) : curshorsku);
...
function createSkuCodeCell(rowIndex) {
    var cell = createCell();
    var skuCodeCell = $(String.format("<input type=\"text\" class=\"skuItem_SkuCode\" id=\"skuCode_{0}\" disabled='disabled' \/>", rowIndex));
    //if ($("#ctl00_contentHolder_txtSku").val().length > 0) $(skuCodeCell).val($("#ctl00_contentHolder_txtSku").val() + "-" + rowIndex);
    if ($("#ctl00_contentHolder_txtSku").val().length > 0) $(skuCodeCell).val();
    cell.append(skuCodeCell);
    return cell;
}

function createUploadCell(rowIndex) {
    var cell = $("<td align=\"center\" style=\"width:100px\"><\/td>");
    var skuid = getRowIdentity(rowIndex);
    var uploadCell = $(String.format("<img id=\"skuimage_{0}\" src=\"\" onmouseover=\"over('skuimage_{0}')\" onmouseout=\"out()\" style=\"width:20px;height:20px;display:none\"><span class=\"submit_tiajia\"><a href=\"javascript:void(0)\" onclick=\"UploadSKUimg('{1}','{0}');\">上传</a></span> ", rowIndex, skuid));
    //var uploadCell = $(String.format("<select id=\"skuimage_{0}\"><option value=\"0\">--请选择--</option><option value=\"1\">商品图片1</option><option value=\"2\">商品图片2</option><option value=\"3\">商品图片3</option><option value=\"4\">商品图片4</option><option value=\"5\">商品图片5</option></select>", rowIndex));
    cell.append(uploadCell);
    return cell;
}
...
//显示图片
function over(imgid) {
    var mouseXY = mousePos(window.event);
    //alert(mouseXY.x);
    //alert(mouseXY.y);
    $('#divImage').css("display", "");
    $("#divImage").css({ top: mouseXY.y - 280, left: mouseXY.x - 310 }); //给显示层定义CSS--当前计算出的位置
    $('#imgbig').attr("src", $("#" + imgid).attr("src"));
    $("#divImage").stopAll().pause(300).fadeTo(200, 1); //显示效果 
}
...
function doSubmit() {
    ...
    if (skuEnabled) {
        ...
        if (!checkGBCode()) return false;
        if (!checkShortSku()) return false;
...
function loadAttributes() {
    ...
    $.each($(".attributeItem"), function (i, att) {
        ...
        if (usageMode == "0") {
            ...
            if ($("#txtvalueEx" + attributeId).length > 0) {
                if ($("#txtvalueEx" + attributeId).val() != "" && $("#txtvalueEx" + attributeId).css("display") != "none") {
                    itemXml += String.format("<attValue valueId=\"{0}\" CUS_Name=\"{1}\" \/>", $("#attribute" + attributeId).val(), $("#txtvalueEx" + attributeId).val());
                }
                else {
                    itemXml += String.format("<attValue valueId=\"{0}\" CUS_Name=\"{1}\" \/>", $("#attribute" + attributeId).val(), "");
                }
...
function loadSkus() {
    ...
    $.each($(".SpecificationTr"), function (i, valitem) {
        ...
        var skuimg = $("#skuimage_" + rowIndex).attr("src");
        //var skuimg = $("#skuimage_" + rowIndex).val();
        var barcode = $("#barcode_" + rowIndex).val();
        var shortsku = $("#ShortSku_" + rowIndex).val();
        var GBCode = $("#GBCode_" + rowIndex).val();
        var itemXml = String.format("<item skuCode=\"{0}\" salePrice=\"{1}\" costPrice=\"{2}\" purchasePrice=\"{3}\" qty=\"{4}\" alertQty=\"{5}\" weight=\"{6}\" barcode=\"{8}\" shortsku=\"{9}\" gbcode=\"{10}\"  skuimage=\"{7}\">", skuCode, salePrice, costPrice, purchasePrice, qty, alertQty, weight, skuimg, barcode, shortsku, GBCode);
        itemXml += "<skuFields>";
        for (i = 0; i < cellFields.length; i++) {
            var obj = $(String.format("#skuDisplay_{0}_{1}", rowIndex, cellFields[i]));
            itemXml += String.format("<sku attributeId=\"{0}\" valueId=\"{1}\" value=\"{2}\" AttrType=\"{3}\" \/>", cellFields[i], obj.attr("valueId"), obj.html(), obj.attr("AttrType"));
        }
        itemXml += "<\/skuFields>";
    ...
    var skuxmlobj = $(".SpecificationTr");
    if (!!skuxmlobj && skuxmlobj.length != 0)
...
function checkShortSku() {
    var validated = true;
    $.each($(".skuItem_ShortSku"), function () {
        var val = $(this).val();
...
function onGBCodeblur(gbcode) {
    ...
    if (gbcode.length != 13) {
        ...
        $.ajax({
            url: '/API/CommonHandler.ashx?op=CheckGBCode',
            dataType: 'json',
            type: 'GET',
            async: false,
            data: { GBCode: gbcode, productid: pid },
            error: function (xm, msg) {
                alert(msg);
            },
            success: function (data) {
...
function checkGBCode() {
    var validated = true;
    $.each($(".skuItem_GBCode"), function () {
        var val = $(this).val();
...
function checkSalePrice() {
    return checkPrice($(".skuItem_SalePrice"), true, "批发价");
}
...
function checkWeight() {
    ...
    return checkNumber($(".skuItem_Weight"), true, "重量", 1);
}
    ...
    $.each(inputItems, function () {
        ...
        if (val.length > 0) {
            ...
            if (!((num >= 0.01) && (num <= 10000000))) {
                alert(String.format("商品规格的{0}要求大于0小于10000000！", priceName));
                $(this).focus();
                validated = false;
                return false;
            }
...
function checkNumber(inputItems, required, numberName, min, max) {
    var validated = true;
    ...
    $.each(inputItems, function () {
        ...
        if (val.length > 0) {
            ...
            if (!!min && num < min) {
                alert(String.format("商品规格的{0}不能小于{1}！", numberName, min));
                $(this).focus();
                validated = false;
                return false;
            }
            if (!!max && num > max) {
                alert(String.format("商品规格的{0}不能大于{1}！", numberName, max));
                $(this).focus();
                validated = false;
                return false;
            }
