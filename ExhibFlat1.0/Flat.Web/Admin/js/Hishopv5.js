            ...
            for (var chked = 0; chked < areaItemsValue[arrLength].length; chked++) {
                ...
                if (verifyValue == true) {
                    for (x = 0; x < areaItemsValueObject.length; x++) {
                        var a = areaItemsValueObject[x].value.split(',').sort(function compare(a, b) { return a - b; }).join(",");
                        if (a == areaItemsValue[arrLength][chked].sort(function compare(a, b) { return a - b; }).join(",")) {
...
function compArr(a, b) {
    if (a.length == b.length) return a.sort(function compare(x, y) { return x - y; }).join("") == b.sort(function compare(x, y) { return x - y; }).join("");
    var arr = a.slice(0);
    a = a.sort(function compare(x, y) { return x - y; }).join(",");
    b = b.sort(function compare(x, y) { return x - y; }).join(",");
