...
function DialogFrame(url, title_tip, w_width, h_height) {
    ...
    if (tmpwidth != 0) {
        if (tmpheight != 0) {
            art.dialog.open(url, { title: title_tip, width: tmpwidth, height: tmpheight,lock:true }, true);
        } else {
            art.dialog.open(url, { title: title_tip, width: tmpwidth, lock: true }, true);
        }
    } else{
        if (tmpheight != 0) {
            art.dialog.open(url, { title: title_tip, height: tmpheight, lock: true }, true);
        } else {
            art.dialog.open(url, { title: title_tip, lock: true }, true);
        }
    }
}
...
function getTypeArrayText(keyname, vals) {
    switch ($("#" + keyname).attr("type")) {
        case "checkbox":
            var che;
            if (vals == "true" || vals == true) {
                che = true;
            }
            else {
                che = false;
            }
            $("#" + keyname).prop("checked", che);
            break;
        case "radio":
            $("#" + keyname).prop("checked", vals);
            break;
        default:
...
function setShowText(parmajson) {
    for (var keyname in parmajson) {
        switch ($("#" + keyname).attr("type")) {
            case "checkbox":
                parmajson[keyname] = $("#" + keyname).prop("checked");
                break;
            case "radio":
                parmajson[keyname] = $("#" + keyname).prop("checked");
                break;
