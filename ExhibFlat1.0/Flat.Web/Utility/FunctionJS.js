...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        if (tabs_container.find('li').length > 5) {
            showFaceMsg("为保证系统效率,只允许同时运行6个功能窗口,请关闭一些窗口后重试！");
            top.$(".navigation").show();
            return false;
        }
        Loading(true);
...
function delConfigReloadTarget(url, parm, count, target) {
    DeleteConfigReloadTarget(url, parm, "注：删除操作不可恢复，您确定要继续么？", count, target);
}
function DeleteConfigReloadTarget(url, parm, Msg, target, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
...
function delConfigReloadTarget1(url, parm, count, target) {
    DeleteConfigReloadTarget1(url, parm, "注：删除操作不可恢复，您确定要继续么？", count, target);
}
function DeleteConfigReloadTarget1(url, parm, Msg, target, count) {
    if (count == undefined) {
        count = 1;
    }
    showConfirmMsg(Msg, function (r) {
...
//=================动态菜单tab标签========================
function AddTabMenu(tabid, url, name, Isclose, IsReplace, IsMenu) {
    if (IsMenu == false) { }
    else
        SetSystemId(tabid);
    if (url == "" || url == "#") {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        ContentPannel.find('iframe').hide();
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','true')\"><a href=\"javascript:;\">" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','" + img + "','false')\"><a><img src=\"/Themes/Images/32/" + img + "\" width=\"20\" height=\"20\">" + name + "</a></li>");
        }
        ContentPannel.append("<iframe id=\"tabs_iframe_" + tabid + "\" name=\"tabs_iframe_" + tabid + "\" height=\"100%\" width=\"100%\" src=\"" + url + "\" frameBorder=\"0\"></iframe>");
...
function GetWebControlsByName(element) {
    var reVal = "";
    var value1 = "", value2 = "", value3 = "", value4 = "", value5 = "", value6 = "", value7 = "", value8 = "", value9="",velue10="";
    ...
    $("input[name='ShowNum']").each(function (i) {
        if ($(this).val() != null & $(this).val() != "") {
            value4 += $(this).val() + ",";
        }
    });
    ...
    $("input[name='Volume']").each(function (i) {
        if ($(this).val() != null & $(this).val() != "") {
            value6 += $(this).val() + ",";
        }
    });
    $("input[name='TransactionAmount']").each(function (i) {
        if ($(this).val() != null & $(this).val() != "") {
            value10 += $(this).val() + ",";
        }
    });
    $("input[name='StatisticsTime']").each(function (i) {
        if ($(this).val() != null & $(this).val() != "") {
            value7 += $(this).val() + ",";
        }
    });
    $("select[name='ShopId']").each(function (i) {
        if ($(this).val() != null & $(this).val() != "") {
            value8 += $(this).val() + ",";
        }
    });
    $("select[name='catsId']").each(function (i) {
        if ($(this).val() != null & $(this).val() != "") {
            value9 += $(this).val() + ",";
        }
    });
    reVal = '"catsId":"' + $.trim(value9) + '","Pname":"' + $.trim(value1) + '",' + '"pId":"' + $.trim(value2) + '",' + '"Price":"' + $.trim(value3) + '",' + '"ShowNum":"' + $.trim(value4) + '",' + '"PV":"' + $.trim(value5) + '",' + '"Volume":"' + $.trim(value6) + '",' + '"StatisticsTime":"' + $.trim(value7) + '","ShopId":"' + $.trim(value8) + '","TransactionAmount":"'+$.trim(velue10)+'"'
   // reVal = reVal.substr(0, reVal.length - 1);
   //alert(reVal);
    return jQuery.parseJSON('{' + reVal + '}');
}
...
function Alert(Msg) {
    showTipsMsg(Msg, '4000', '4'); windowload();
}
/// <summary>
/// 默认成功提示，刷新父窗口函数关闭页面
/// </summary>
...
/// </summary>
/// <param name="Msg">显示消息</param>
function AlertParmCallback(Msg) {
    showTipsMsg(Msg, '4000', '4');
    top.$('#' + Current_iframeID())[0].contentWindow.target_right.windowload();
    OpenClose();
}
/// <summary>
/// 默认成功提示，刷新父窗口函数 当前页面不关闭
/// </summary>
/// <param name="Msg">显示消息</param>
function AlertCurrent(Msg) {
    showTipsMsg(Msg, '4000', '4');
    //top.$('#' + Current_iframeID())[0].contentWindow.windowload();
}
/// <summary>
...
function Alert_Error(Msg) {
    showTipsMsg(Msg, '5000', '5');
}
/// <summary>
/// 默认警告提示
/// </summary>
/// <param name="Msg">显示消息</param>
function Alert_Wern(message) {
    showTipsMsg(message, '5000', '3');
}
//动态加载工具栏权限按钮
function Tools_bar() {
    $(".tools_bar").html("");
    getAjax("/Base/LoadButtonPermission", "", function (data) {
        $(".tools_bar").html(data);
    })
    /*****************按钮********************************/
    $(".tools_bar .tools_btn").hover(function () {
