namespace ExhibFlat.UI.Common.Controls
{
    ...
    public class ImageUploader : WebControl
    {
        ...
        //图片服务器地址
        private string IMAGEURL = System.Configuration.ConfigurationManager.AppSettings["ImageUrl"].ToString();

        public ImageUploader()
        {
            this.UploadType = ExhibFlat.UI.Common.Controls.UploadType.Product;
        }
        ...
        private bool CheckFileExists(string imageUrl)
        {
            ...
            if (imageUrl.ToLower().IndexOf("http://") == 0 || imageUrl.ToLower().IndexOf("https://") == 0)
            {
                Regex re = new Regex("^(http|https)://[^/]+/");
                String url = re.Replace(imageUrl, "", 1);
                return File.Exists(this.Page.Request.MapPath("/" + url));//File.Exists(this.Page.Request.MapPath(Globals.ApplicationPath + imageUrl));
            }
            return true;
        }
        ...
        protected override void CreateChildControls()
        {
            ...
            this.Controls.Clear();
            string webResourceUrl = this.Page.ClientScript.GetWebResourceUrl(base.GetType(), "ExhibFlat.UI.Common.Controls.ImageUploader.images.upload.png");
            ...
            if (this.Page.Header != null && this.Page.Header.FindControl("uploaderStyle") == null)
            {
                WebControl control6 = new WebControl(HtmlTextWriterTag.Link);
                control6.Attributes.Add("rel", "stylesheet");
                control6.Attributes.Add("href", this.Page.ClientScript.GetWebResourceUrl(base.GetType(), "ExhibFlat.UI.Common.Controls.ImageUploader.css.style.css"));
        ...
        public override void RenderControl(HtmlTextWriter writer)
        {
            ...
            {
                string script = string.Format("<script src=\"{0}\" type=\"text/javascript\"></script>", this.Page.ClientScript.GetWebResourceUrl(base.GetType(), "ExhibFlat.UI.Common.Controls.ImageUploader.script.upload.js"));
        ...
        public string UploadedImageUrl
        {
            ...
            set
            {
                if (this.CheckFileExists(value))
                {
                    ...
                    {
                        this.uploadedImageUrl = IMAGEURL.TrimEnd('/') + value;
                        this.thumbnailUrl40 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs40/40_");
                        this.thumbnailUrl60 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs60/60_");
                        this.thumbnailUrl100 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs100/100_");
                        this.thumbnailUrl160 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs160/160_");
                        this.thumbnailUrl180 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs180/180_");
                        this.thumbnailUrl220 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs220/220_");
                        this.thumbnailUrl310 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs310/310_");
                        this.thumbnailUrl410 = IMAGEURL.TrimEnd('/') + value.Replace("/images/", "/thumbs410/410_");
                    }
                    else
                    {
        ...
        public ExhibFlat.UI.Common.Controls.UploadType UploadType { get; set; }
    }
}
