...
namespace ExhibFlat.UI.Common.Controls
{
    ...
    public abstract class HtmlTemplatedWebControl : TemplatedWebControl
    {
        ...
        {
            //String SSOPassport = ConfigurationManager.AppSettings["SSOPassport"].TrimEnd('/');
            //String FindPassword = SSOPassport + "/userinfo/FindPassword";
            //String Register = SSOPassport + "/Admin/Register";
            if (!this.SkinFileExists)
            {
                return null;
            }
            StringBuilder builder =
                new StringBuilder(File.ReadAllText(this.Page.Request.MapPath(this.SkinPath), Encoding.UTF8));
            if (builder.Length == 0)
            {
                return null;
            }
            ...
            //builder.Replace("url:ForgotPassword", FindPassword);
            //builder.Replace("url:register", Register);
            builder.Insert(0,
                "<%@ Register TagPrefix=\"UI\" Namespace=\"ASPNET.WebControls\" Assembly=\"ASPNET.WebControls\" %>" +
                Environment.NewLine);
            builder.Insert(0,
                "<%@ Register TagPrefix=\"UI2\" Namespace=\"ASPNET.WebControls2\" Assembly=\"ASPNET.WebControls2\" %>" +
                Environment.NewLine);
            builder.Insert(0,
            ...
            for (int i = matchs.Count - 1; i >= 0; i--)
            {
                ...
                builder.Insert(startIndex,
                    Globals.GetSiteUrls()
                        .UrlData.FormatUrl(matchs[i].Groups["UrlName"].Value.Trim(),
                            new object[] {matchs[i].Groups["Param"].Value}));
            }
        ...
        {
            get { return !string.IsNullOrEmpty(this.SkinName); }
        }
