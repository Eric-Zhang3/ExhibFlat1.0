namespace ExhibFlat.Membership.Core
{
    ...
    public static class HiMembership
    {
        ...
        public static bool PasswordIsMembershipCompliant(string newPassword, out string errorMessage)
        {
            ...
            {
                errorMessage = string.Format(CultureInfo.InvariantCulture, "密码太短，最少需要 {0} 个字符",
                    new object[] {Membership.MinRequiredPasswordLength.ToString(CultureInfo.InvariantCulture)});
                return false;
            }
            int num3 = 0;
            ...
            {
                errorMessage = string.Format(CultureInfo.InvariantCulture, "密码包含的特殊字符太少, 最少要包含 {0} 个特殊字符",
                    new object[]
