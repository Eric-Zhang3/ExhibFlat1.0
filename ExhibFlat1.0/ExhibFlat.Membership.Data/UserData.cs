namespace ExhibFlat.Membership.Data
{
    ...
    public class UserData : MemberUserProvider
    {
        ...
        public override bool ChangePasswordQuestionAndAnswer(string username, string newQuestion, string newAnswer)
        {
            ...
            DbCommand sqlStringCommand =
                this.database.GetSqlStringCommand(
                    "UPDATE aspnet_Users SET PasswordQuestion = @PasswordQuestion, PasswordAnswer = @PasswordAnswer WHERE LOWER(@Username) = LoweredUserName");
        ...
        //public override CreateUserStatus CreateMembershipUser(HiMembershipUser userToCreate, string passwordQuestion,
        ...
        //    #region  Create User 修改 by Yongjin.C

        //    ExhibFlat.Membership.ASPNETProvider.SqlMembershipProvider s =
        //        new ExhibFlat.Membership.ASPNETProvider.SqlMembershipProvider();
        //    MembershipUser user = s.CreateUser(userToCreate.UserId, userToCreate.Username, userToCreate.Password,
        //        userToCreate.Email);
        ...
        //        DbCommand sqlStringCommand =
        //            this.database.GetSqlStringCommand(
        //                "UPDATE aspnet_Users SET IsAnonymous = @IsAnonymous, IsApproved = null, PasswordQuestion = @PasswordQuestion, PasswordAnswer = @PasswordAnswer, Gender = @Gender, BirthDate = @BirthDate, UserRole = @UserRole WHERE UserId = @UserId");
        ...
        {
            DbCommand sqlStringCommand =
                this.database.GetSqlStringCommand("SELECT @UserId = UserId FROM aspnet_Users WHERE IsAnonymous = 1");
            ...
            out int passwordFormat, out string passwordSalt)
        {
            ...
            this.database.AddInParameter(storedProcCommand, "UserName", DbType.String, username);
            this.database.AddInParameter(storedProcCommand, "UpdateLastLoginActivityDate", DbType.Boolean,
        ...
        private void GetPasswordWithFormat(string username, bool updateLastLoginActivityDate, out int status,
            out string password, out int passwordFormat, out string passwordSalt, out int failedPasswordAttemptCount,
        ...
        public override string GetUsernameWithOpenId(string openId, string openIdType)
        {
            ...
            DbCommand sqlStringCommand =
                this.database.GetSqlStringCommand(
                    "SELECT UserName FROM aspnet_Users WHERE LOWER(OpenId)=LOWER(@OpenId) AND LOWER(OpenIdType)=LOWER(@OpenIdType)");
        ...
        public override bool UpdateMembershipUser(HiMembershipUser user)
        {
            ...
            DbCommand sqlStringCommand =
                this.database.GetSqlStringCommand(
                    "UPDATE aspnet_Users SET MobilePIN = @MobilePIN, Gender = @Gender, BirthDate = @BirthDate WHERE UserId = @UserId");
        ...
        public override bool ValidatePasswordAnswer(string username, string answer)
        {
            ...
            DbCommand sqlStringCommand =
                this.database.GetSqlStringCommand(
                    "SELECT UserId FROM aspnet_Users WHERE LOWER(@Username) = LoweredUserName AND (PasswordAnswer = @PasswordAnswer OR (PasswordQuestion IS NULL AND PasswordAnswer IS NULL))");
            this.database.AddInParameter(sqlStringCommand, "Username", DbType.String, username);
            this.database.AddInParameter(sqlStringCommand, "PasswordAnswer", DbType.String, str2);
        ...
        //对User表插入供应商ID
        public override bool UpdateSupplierIdByUser(int UserId, string SupplierId, string RelName, string Phone)
        {
            DbCommand sqlStringCommand =
                this.database.GetSqlStringCommand(
                    "update dbo.aspnet_Users set SupplierId=@SupplierId,RelName=@RelName,Phone=@Phone WHERE UserId=@UserId");
            this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, UserId);
            this.database.AddInParameter(sqlStringCommand, "SupplierId", DbType.String,
                SupplierId);
            this.database.AddInParameter(sqlStringCommand, "RelName", DbType.String, RelName);
            this.database.AddInParameter(sqlStringCommand, "Phone", DbType.String, Phone);
        ...
        public override string ValiCompanyName(string cname)
        {
            DbCommand sqlStringCommand =
                this.database.GetSqlStringCommand(
                    "select CompanyName  from dbo.Hishop_Suppliers where CompanyName=@CompanyName");
            this.database.AddInParameter(sqlStringCommand, "CompanyName", DbType.String, cname);
            object obj2 = this.database.ExecuteScalar(sqlStringCommand);
            if (obj2 != null)
            {
        ...
        //验证营业号的唯一
        public override bool CheckBnessCode(string code)
        {
         bool b=false;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select BnessCode from dbo.Hishop_Suppliers where BnessCode='" + code+"'");
            object obj2 = this.database.ExecuteScalar(sqlStringCommand);
            if (obj2 != null)
            {
                b = true;
