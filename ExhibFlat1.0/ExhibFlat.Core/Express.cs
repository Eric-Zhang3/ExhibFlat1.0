    ...
    public static class Express
    {
        public static string GetDataByKuaidi100(string computer, string expressNo)
        {
            ...
            if (!string.IsNullOrEmpty(str2))
            {
                ...
                //}
                document.Load(str2);
                XmlNodeList nodeList = document.SelectNodes("/companys/company");
                if ((nodeList != null) && (nodeList.Count > 0))
                {
                    foreach (XmlNode node in nodeList)
                    {
                        if (node.Attributes["name"].Value == computer)
                        {
                            com = node.Attributes["Kuaidi100Code"].Value; continue;
                        }
                    }
                }
            }
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(string.Format("http://api.kuaidi100.com/api?com={0}&nu={1}&show=2&id={2}", com, expressNo, innerText));
